...
#include <ctype.h>
#include <string.h>
const int SIZE = 8; /* Board size - must be even */
const char comp_c = '@'; /* Computer's counter */
const char player_c = 'O'; /* Player's counter */
/* Function prototypes */
void computer_move_supereasy(char board[][SIZE], bool moves[][SIZE], char player);
void computer_move_easy(char board[][SIZE], bool moves[][SIZE], char player);
...
int main(void)
{
    ...
    printf(" You will be white - (%c)\n I will be black - (%c).\n",
           player_c, comp_c);
    printf("Select a square for your move by typing a digit for the column\n "
           "and a letter for the row with no spaces between.(e,g e3)\n");
    ...
    {
        for(;;){
        printf("How good are you?\n1.I suck\n2.I'm soso\n3.I've played it for 23 years so you tell me.\nPlease enter 1 2 or 3:");
        scanf("%d",&difficulty);
        ...
        do
        {
            ...
            { /* It is the player's turn */
                if(valid_moves(board, moves, player_c))
                {
                    ...
                    for(;;)
                    {
                        int x = 0;
                        char y = 0;
                        printf("\nPlease enter your move (column row): ");
                        scanf(" %c%d", &y, &x); /* Read input */
            ...
            { /* It is the computer's turn */
                if(valid_moves(board, moves, '@')) /* Check for valid moves */
                {
                    ...
                    switch (difficulty) {
                        case 1:
                        computer_move_supereasy(board, moves, '@');
                        break;
                        
                        case 2:
                        computer_move_easy(board, moves, '@');
                        break;
        ...
        printf("Computer wins, good game!\n\n");
        else
        {
        if(comp_score<user_score)
            printf("Congratulation!! You beat the computer!\n\n");
        else
...
void display(char board[][SIZE])
{
    ...
    char col_label1 = 'a'; /* Column label */
    printf("\n "); /* Start top line */
    for(int col = 0 ; col<SIZE ;col++)
    printf("   %c", col_label1+col); /* Display the top line */
    printf("\n"); /* End the top line */
...
{
    int r = 0;
    int c = 0;/* Go through all valid moves */
    for(int row = 0; row < SIZE; row++)
    for(int col = 0; col < SIZE; col++)
    {
        if(moves[row][col] )
